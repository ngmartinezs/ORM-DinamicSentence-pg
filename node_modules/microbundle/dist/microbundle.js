...
const getSizeInfo = function (code, filename, raw) {
  try {
    ...
    return Promise.resolve(Promise.all([gzipSize(code).catch(() => null), brotliSize(code).catch(() => null)])).then(function ([gzip, brotli]) {
...
const getConfigFromPkgJson = function (cwd) {
  try {
    ...
    const _temp = _catch(function () {
      return Promise.resolve(readFile(path.resolve(cwd, 'package.json'), 'utf8')).then(function (packageJson) {
...
const getOutput = function ({
  ...
  try {
    ...
    return Promise.resolve(_main$match ? _temp7(_main$match) : Promise.resolve(isDir(main)).then(_temp7));
  } catch (e) {
    return Promise.reject(e);
  }
};
...
const getInput = function ({
  ...
  try {
    function _temp6(_isDir) {
      function _temp5(_jsOrTs) {
        function _temp4(_jsOrTs2) {
          _concat.call([], _temp3 ? _jsOrTs2 : _jsOrTs2 || module).map(file => glob(file)).forEach(file => input.push(...file));
    ...
    const _concat = [].concat,
          ...
          _temp2 = _temp3 || source && (Array.isArray(source) ? source : [source]).map(file => path.resolve(cwd, file));
      ...
      return Promise.resolve(getInput({
        ...
        return Promise.resolve(getOutput({
          ...
          return Promise.resolve(getEntries({
            ...
            let formats = (options.format || options.formats).split(','); // always compile cjs first if it's there:
...
function getMain({
  ...
  mainsByFormat.umd = replaceName(pkg['umd:main'] || 'x.umd.js', mainNoExtension);
